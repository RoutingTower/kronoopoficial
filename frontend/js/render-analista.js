...
// em render-supervisor.js), só que escopado ao TIME inteiro do mesmo
// supervisor (não só o próprio analista) — pedido explícito pra ajudar nas
// análises em conjunto. Sem seletor de "quem" (picker vazio): mostra o
// time todo de uma vez, mesmo padrão de myAnalistas em renderSupervisor().
function analistaResultadoSPR(){
  const meuSupervisorId = userById(session.userId)?.supervisorId;
  const meuTime = DB.users.filter(u=>u.role==='analista' && u.supervisorId===meuSupervisorId);
  return sprResultadoBody(meuTime, '');
}
